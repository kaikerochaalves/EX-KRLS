...
l_alpha = [0.95, 0.99]
l_beta = [0.3, 0.7, 0.9, 0.99]
l_lambda1 = [1E-4, 1E-2]
l_q = [1E-4, 1E-2]
l_M = [50, 100, 150, 200]
l_kernel_width = [0.1, 0.5, 1, 10]
...
columns = ['alpha', 'beta', 'lambda1', 'q', 'M', 'kernel_width', 'RMSE', 'NDEI', 'MAE']
...
for alpha in l_alpha:
    for beta in l_beta:
        for lambda1 in l_lambda1:
            for q in l_q:
                for M in l_M:
                    for kernel_width in l_kernel_width:
                            
                        print(f"alpha = {alpha} and beta = {beta} and lambda = {lambda1} and q = {q} and M = {M} and kernel_width = {kernel_width}")
                    
                        # Initializing the model
                        model = EX_KRLS(alpha = alpha, beta = beta, lambda1 = lambda1, q = q, M = M, kernel_width = kernel_width)
                        # Train the model
                        ...
                        NewRow = pd.DataFrame([[alpha, beta, lambda1, q, M, kernel_width, RMSE, NDEI, MAE]], columns = columns)
